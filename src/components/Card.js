...
const Card = ({ movie }) => {
    ...
    return (
        <div className='card'>
            ...
            <h2>{movie.title}</h2>
            {movie.release_date ? <h5>Sorti le : {formaterDate(movie.release_date)}</h5> : "" }
            ...
            {movie.genre_ids ? 
            (<div className="btn" onClick={() => addToStorage()} >Ajouter aux coups de coeur</div>)
            : (<div className="btn" onClick={() => {removeToStorage(); window.location.reload()} }>Supprimer de la liste</div>)
          }
        </div>
    );
};
